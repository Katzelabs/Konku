...
**Terhapus is not forever** (D-069). A daily job in the server process removes notes and cards deleted more than 30 days ago — soft delete used to mean nothing ever left the database, which stopped being viable once the quota counted only live rows. **A card that was ever reviewed or examined is exempt and kept indefinitely**: `review_logs` and `exam_attempt_cards` have no FK to `cards` (D-050), so only the query's predicate stops a purge orphaning them. The window is stated in every delete dialog, both Terhapus views and the privacy policy — a trash that empties itself silently would be the disappearance this product exists to prevent.
