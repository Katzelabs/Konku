...
**`07` L1–L9 are complete, and so is `08-review.md`. Next: `04-ship.md`**, then `07` L10.
...
**The two index screens split in two** (D-074's sibling change). `/notes` and `/cards` are a full-width header over a two-column grid: search, filters and the list in the left panel, the open item's preview in the right. The preview used to be a `fixed inset-y-0 right-0` panel that App rendered beside the routes and pushed the page out of the way of; it is a column of the page now, rendered by the index itself, which is what lets the two share a header and a height. Peeking is still a URL and still keyed off the background location — only where the panel is mounted changed. Below `lg` the two swap: list until something is open, then preview. `PeekMode` still calls it `side` because that is what every machine's localStorage already says.

**Categories have a colour and a settings screen** (D-074, migration `00011`). `/categories` sits beside `/domains` under Pengaturan and does rename, recolour, archive and delete. The colour is a **dot on a neutral chip, never a fill** — D-054's confetti objection was about tinted chips beside a domain badge and is still respected. Create-on-type still posts a label and no colour, and posting an existing label returns it unchanged rather than repainting it.
