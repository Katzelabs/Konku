...
| `Flashcard` | A card with two sides, turned over rather than scrolled past (D-080). The peek used to stack question, rule and answer down one column, which rendered a card as a short note with two fields. **Not recall-before-reveal** — D-003 is the server refusing to send `back` with a prompt; here the whole card is already in hand and the flip is how you handle it, so both faces stay mounted. The faces share **one grid cell**, so the card is as tall as its taller side and does not resize mid-turn. `--animate-duration-flip` (300ms) rather than `calm`: below ~250ms the turn is not seen and the answer simply appears. Neither face is `bg-card` — it lives inside a peek panel and a dialog, both already `bg-card` — so the front is `surface`, the back `muted`, and hover moves the border. The hidden face is `inert`, or a tab lands on the back of a card you are reading the front of. |
